...
bool init_player_ent(ZF4Scene* scene, ZF4EntID entID, ZF4GamePtrs* gamePtrs) {
    ZF4Ent* playerEnt = zf4_get_ent(&scene->entManager, entID);
...
bool player_ent_tick(ZF4Scene* scene, ZF4EntID entID, ZF4GamePtrs* gamePtrs) {
    ZF4Ent* playerEnt = zf4_get_ent(&scene->entManager, entID);
    PlayerEntExt* playerEntExt = zf4_get_ent_type_ext(&scene->entManager, entID);
    ...
    ZF4Vec2D velTarg = zf4_calc_vec_2d_scaled(moveAxis, calc_move_spd(playerEntExt));
    float velLerp = 0.25f;
    ...
    if (zf4_is_mouse_button_released(ZF4_MOUSE_BUTTON_LEFT)) {
        ...
        ZF4Vec2D hitboxDir = zf4_calc_vec_2d_dir(playerEnt->pos, mouseCamPos);
        ZF4Vec2D hitboxOffs = zf4_calc_vec_2d_scaled(hitboxDir, SWORD_HITBOX_OFFS_DIST);
        ZF4Vec2D hitboxPos = zf4_calc_vec_2d_sum(playerEnt->pos, hitboxOffs);
        ZF4Vec2D hitboxForce = zf4_calc_vec_2d_scaled(hitboxDir, SWORD_HITBOX_STRENGTH);

        ZF4EntID hitboxEntID = zf4_spawn_ent(scene, HITBOX_ENT, hitboxPos, gamePtrs);
        
        ZF4Ent* hitboxEnt = zf4_get_ent(&scene->entManager, hitboxEntID);
        hitboxEnt->scale = (ZF4Vec2D) {SWORD_HITBOX_SIZE, SWORD_HITBOX_SIZE};

        HitboxEntExt* hitboxEntExt = zf4_get_ent_type_ext(&scene->entManager, hitboxEntID);
    ...
    float swordRotOffsTargAbs = SWORD_ROT_OFFS_MAX + (((float)playerEntExt->swordChargeTime / SWORD_CHARGE_TIME_MAX) * SWORD_CHARGE_ROT_OFFS);
    float swordRotOffsTarg = ((playerEntExt->swordRotAxis ? 1 : -1) * swordRotOffsTargAbs);
    ...
    //
    ZF4Vec2D swordOffs = zf4_calc_len_dir_vec_2d(SWORD_OFFS_DIST, playerEntExt->swordRot);
    ZF4Vec2D swordPos = zf4_calc_vec_2d_sum(playerEnt->pos, swordOffs);
