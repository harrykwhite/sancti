...
static bool enemy_ent_collision_filter(ZF4EntID entID, ZF4EntManager* entManager) {
...
bool hitbox_ent_tick(ZF4Scene* scene, ZF4EntID entID, ZF4GamePtrs* gamePtrs) {
    ZF4Ent* ent = zf4_get_ent(&scene->entManager, entID);
    HitboxEntExt* entExt = zf4_get_ent_type_ext(&scene->entManager, entID);

    ZF4EntID enemyEntCollisionIDs[32] = {0};
    int enemyEntCollisionCnt = zf4_get_colliding_ents(enemyEntCollisionIDs, ZF4_STATIC_ARRAY_LEN(enemyEntCollisionIDs), enemy_ent_collision_filter, entID, &scene->entManager);
